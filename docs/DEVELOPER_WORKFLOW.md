...
Canonical filing contract (required):
- Append change record to `docs/changes/CHANGELOG_OPERATIONS.md`
- Append evidence record to `docs/audits/AUDIT_TRAIL.md`
- Use `ops/scripts/log_change.sh` for fast dual logging

**Summary**: PROPOSE → REVIEW → TEST → LOG → EXECUTE → ARCHIVE → COMMIT

Documentation maintenance shortcut:
- For documentation-specific add/edit/delete/version actions, follow `docs/runbooks/DOCUMENTATION_LIFECYCLE_WORKFLOW.md`.
- Start navigation from `docs/README.md`.
...
- Hardware reality: local models run CPU-only on OptiPlex 7090 (i5-10500, 33GB RAM, no discrete GPU VRAM).
- Swap behavior: 128GB NVMe swap exists to preserve completion under pressure; latency may increase but jobs should continue.
- Governance rule: mutating actions require proposal lifecycle visibility (`pending -> approved -> executed`).
- Runtime truth source: use `/api/monitor` and `/api/chat/jobs/status` telemetry instead of inferred progress.
- Thread continuity rule: reuse current thread unless explicit `new_thread=true` is provided.

Operational prompt guidance for swarm agents in Fridays:

- Do not fabricate hardware capabilities (no phantom GPU claims).
- Do not report placeholders as final outputs; mark pending jobs and continue polling.
- Treat ETA as advisory and stage/status as authoritative.
- Surface failures as triage items with proposal IDs when action is required.
