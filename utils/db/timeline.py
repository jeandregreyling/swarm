...
def timeline_append(conv_id, agent, event_type, payload, _conn=None):
    """
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO conv_timeline (conv_id, agent, event_type, payload)
               VALUES (?, ?, ?, ?)""",
            (int(conv_id), str(agent), str(event_type), text),
        )
        if own:
            conn.commit()
            conn.close()
