...
def log_activity(service, event, detail='', severity='info'):
    """Log an activity to the activity_log table. Used by old code paths."""
    conn = get_connection()
    try:
        conn.execute(
            "INSERT INTO activity_log (service, event, detail, severity) VALUES (?,?,?,?)",
            (service, event, detail, severity)
        )
        conn.commit()
    finally:
        conn.close()
