...
function initALMData(refreshRuntime = false) {{
    if (window.__almApplying) return;
    window.__almApplying = true;
    if (window._almData) _applyALMVisibility(window._almData);
    window.__almApplying = false;

    // Refine baked data from authoritative runtime endpoint when available.
    // Guard against fetch storms when dynamic DOM mutations are frequent.
    if (refreshRuntime && typeof fetch === 'function') {{
        const now = Date.now();
        const last = window.__almLastFetchTs || 0;
        if (window.__almFetchInFlight || (now - last) < 10000) return;

        window.__almFetchInFlight = true;
        window.__almLastFetchTs = now;
        fetch('/api/alm/status')
          .then(r => r.json())
          .then(d => {{ if (d && d.ok) {{ window._almData = d; _applyALMVisibility(d); }} }})
          .catch(() => {{}})
          .finally(() => {{ window.__almFetchInFlight = false; }});
    }}
}}

if (document.readyState === 'loading') {{
    document.addEventListener('DOMContentLoaded', () => initALMData(true));
}} else {{
    initALMData(true);
}}

// Keep ALM UI synced when views are opened dynamically.
const _almObs = new MutationObserver(() => initALMData(false));
_almObs.observe(document.documentElement || document.body, {{ childList: true, subtree: true }});

// Refresh ALM runtime state at low frequency for dashboard freshness.
if (!window.__almPollTimer) {{
    window.__almPollTimer = setInterval(() => initALMData(true), 30000);
}}
"""
        return js_code
