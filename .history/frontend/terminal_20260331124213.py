...
def api_chat():
    ...
    def _run_single_agent(selected_agent, prompt, history, transcript):
        response_text = None
        tokens_used = 0
        executor = ThreadPoolExecutor(max_workers=1)
        try:
            if selected_agent in {'gemma', 'llama', 'qwen', 'librarian'}:
                future = executor.submit(orchestrator.ask_agent, selected_agent, prompt)
    ...
    try:
        ...
        for selected_agent in normalized_agents:
            response_text, tokens_used = _run_single_agent(selected_agent, threaded_prompt, history, transcript)
            responses.append({
