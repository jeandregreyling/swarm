...
class ThemeEngine:
    ...
    def __init__(self):
        self.swarm_root = Path(__file__).resolve().parents[1]
    ...
    def _get_core_time_wizard(self):
        """Always resolve Time Wizard from core/time_machine.py to avoid shadow module drift."""
        spec = importlib.util.spec_from_file_location('time_machine', str(self.swarm_root / 'core' / 'time_machine.py'))
        mod = importlib.util.module_from_spec(spec)
        assert spec and spec.loader
        spec.loader.exec_module(mod)
        sys.modules['time_machine'] = mod
        return mod.time_wizard
    
    def get_time_wizard_data(self):
        """Fetch Time Wizard sessions, events, and statistics."""
        try:
            time_wizard = self._get_core_time_wizard()
...
"""
    ...
    def get_alm_data(self):
        ...
        try:
            conn = sqlite3.connect(str(self.swarm_root / 'swarm_memory.db'))
...
function _applyALMVisibility(data) {{
    ...
    if (monitor) {{
        ...
        monitor.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">' +
                            ...
                            'Vortex: <strong>' + (data.time_wizard_active ? 'active' : 'inactive') + '</strong> · ' +
