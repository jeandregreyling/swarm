...
@test('REQ-002', 'POST /api/chat — responds within 5s or returns known timeout error')
def t_chat():
    # The orchestrator hangs on ask_agent() when Ollama is unresponsive.
    # This test uses a 5s timeout. PASS = answered in time. FAIL = hung.
    # Known blocker: if Ollama offline, 500 or timeout expected.
    status, data = post('/api/chat', {'message': 'ping'}, timeout=5)
    if status == 0:
        # Connection timeout — orchestrator is blocking
        raise AssertionError(
            'KNOWN BLOCKER: chat endpoint did not respond within 5s. '
            'Orchestrator ask_agent() has no timeout. '
            'Fix: wrap ask_agent in concurrent.futures.ThreadPoolExecutor with timeout=30.'
        )
    if status == 500:
        raise AssertionError(f'Chat 500: {data}')
    assert status == 200, f'HTTP {status}: {data}'
    assert 'response' in data or 'ok' in data, f'Unexpected response shape: {data}'
    return f'HTTP {status}, response keys: {list(data.keys())}'
...
def t_vortex_dryrun():
    ...
    cp_name = checkpoints[0].get('name') or checkpoints[0].get('label')
    assert cp_name, f'No name/label in checkpoint: {checkpoints[0]}'
...
def t_queue_create():
    ...
    assert entry.get('id') == queue_id or entry.get('queue_id') == queue_id, \
        f'ID mismatch: {entry}'
