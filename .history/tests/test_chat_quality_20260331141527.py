...
def test_skill_then_chat_no_bleed():
    ...
    # Step 2: Verify history is accessed without fridays rows by checking the messages endpoint
    code2, d2 = _req('GET', f'/api/messages?conversation_id={conv_id}', timeout=5)
    if code2 != 200:
...
def test_denied_agent_skill():
    """gemma with deny on 'disk' skill should get 403."""
    # First explicitly deny gemma disk access
