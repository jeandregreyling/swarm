...
WHITELIST = [
    ...
    (r'^ls(\s+-[lahrt]+)?(\s+/home/seven/swarm\S*)?$',   0, 'list files'),
    (r'^find\s+/home/seven/swarm\S*\s+-maxdepth\s+\d+\s+-type\s+[fd](\s+-name\s+\S+)?$', 0, 'find files (bounded)'),
    (r'^cat\s+/home/seven/swarm/sandpits/\S+$',          0, 'read sandpit file'),
    (r'^wc\s+-[lw]\s+/home/seven/swarm/\S+$',            0, 'word/line count'),
    (r'^tail(\s+-n\s+\d+)?\s+/home/seven/swarm/\S+$',    0, 'tail file'),
    (r'^head(\s+-n\s+\d+)?\s+/home/seven/swarm/\S+$',    0, 'head file'),
