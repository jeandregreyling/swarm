  ...
  fetch(`/api/workspace/dir?path=${encodeURIComponent(path || '/home/seven/swarm')}`)
    ...
    .then(data => {
      ...
      if (parentPath) {
        ...
        upRow.innerHTML = '<span style="min-width:20px;">📁</span><span style="flex:1;">.. (parent)</span>';
      ...
      data.entries.forEach(entry => {
        ...
        let actionBtns = '';
        if (entry.type === 'file') {
          actionBtns = `<button title="Preview" class="file-btn-preview" data-path="${_escHtml(entry.path)}" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">👁</button>
            <button title="Open in terminal" class="file-btn-terminal" data-path="${_escHtml(entry.path)}" data-isdir="false" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">⌨</button>
            <button title="Search in file" class="file-btn-grep" data-path="${_escHtml(entry.path)}" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">🔎</button>
            <button title="Copy path" class="file-btn-copy" data-path="${_escHtml(entry.path)}" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">📋</button>`;
        } else {
          actionBtns = `<button title="Open in terminal" class="file-btn-terminal" data-path="${_escHtml(entry.path)}" data-isdir="true" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">⌨</button>
            <button title="Copy path" class="file-btn-copy" data-path="${_escHtml(entry.path)}" style="background:transparent;border:1px solid var(--border);color:var(--text-dim);border-radius:4px;padding:2px 6px;cursor:pointer;font-size:10px;">📋</button>`;
        }
...
function _filesShowDiffModal(filePath, originalContent, newContent) {
  ...
  content.innerHTML = `
    <div>
      <div style="font-size:12px;font-weight:700;color:var(--text);margin-bottom:8px;">📋 Review Changes</div>
