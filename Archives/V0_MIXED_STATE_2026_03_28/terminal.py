...
_DOCS_DIR = _os.path.join(_os.path.dirname(__file__), 'swarm_docs')
...
def api_terminal_run():
    ...
    from fridays.skills import call as skill_call, parse_skill_command
    from database import log_activity

    parsed = parse_skill_command(cmd)
    if parsed:
        skill_name, args = parsed
        ok, output = skill_call(skill_name, args=args, agent='ghost')
        log_activity('terminal', 'terminal_skill', f'{skill_name}: {cmd[:60]}')
        return jsonify({'output': output, 'ok': ok})

    ok, output = skill_call('shell', args=cmd, agent='ghost')
    log_activity('terminal', 'terminal_shell', cmd[:60])
    return jsonify({'output': output, 'ok': ok})
