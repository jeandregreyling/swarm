...
### BUG-TRUST-TELEGRAM-UNKNOWN-BYPASS: Unknown Telegram users reached pipeline
- **Status:** ✅ FIXED
- **Date:** 2026-03-31
- **File:** `fridays/telegram_bot.py`
- **Issue:** Unknown Telegram senders were able to flow into pipeline execution path before moderation trust review.
- **Fix:** Enforced hold-and-notify behavior for unknown senders and preserved trust classification boundaries before pipeline execution.
- **Validation:** `tests/test_telegram_trust.py` pass; unknown-user path now returns moderation hold message and Ghost notification.

### BUG-ALM-ENDPOINT-GAPS: Mutation endpoints missing proposal gate coverage
- **Status:** ✅ FIXED
- **Date:** 2026-03-31
- **Files:** `frontend/terminal.py`, `tests/test_alm_gate_endpoints.py`
- **Issue:** Selected mutation endpoints were not consistently enforcing ALM proposal gating.
- **Fix:** Applied `_alm_gate_or_response` protection to sender mutation endpoints, simulation endpoint, and restore path when `dry_run=false`.
- **Validation:** `tests/test_alm_gate_endpoints.py` pass through `tests/run_uat_gate.py`.

### FEATURE-ACCESS-MANAGER-ONBOARD: Controlled onboarding route for second-user workflow
- **Status:** ✅ IMPLEMENTED
- **Date:** 2026-03-31
- **Files:** `frontend/terminal.py`, `tests/test_manager_onboarding_api.py`
- **Capability:** Added `POST /api/access/manager/onboard` with dry-run planning and execution path, aligned to ALM review process.
- **Validation:** API dry-run response validated and unit tests passing.
