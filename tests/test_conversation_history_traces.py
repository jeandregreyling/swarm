...
def test_conversation_messages_recover_missing_conversation_metadata(tmp_path, monkeypatch):
    db_path = tmp_path / "swarm_memory.db"
    monkeypatch.setenv("SWARM_DB_PATH", str(db_path))
    monkeypatch.setenv("SWARM_ROOT", str(ROOT))
    if str(ROOT) not in sys.path:
    ...
    trace = [{"text": "orphaned runtime job (watchdog)"}]
    conv_id = 2577
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO chat_jobs
                 (job_id, conversation_id, agent, status, runtime_class, eta_seconds,
                  started_at, updated_at, stage, error, elapsed_ms, stage_trace_json)
               VALUES (?, ?, 'gemma', 'failed', 'local', 85,
                  '2026-05-08T05:28:01Z', '2026-05-08T05:35:42Z',
                  'failed', 'server restarted - job lost', 0, ?)""",
            ("chatjob-orphan-history", conv_id, json.dumps(trace)),
        )
        conn.commit()
    ...
    assert resp.status_code == 200
    assert body["conv"]["id"] == conv_id
    assert body["conv"]["source"] == "recovered"
    assert body["messages"] == []
    assert body["jobs"][0]["job_id"] == "chatjob-orphan-history"
    assert body["jobs"][0]["stage_trace"] == trace
